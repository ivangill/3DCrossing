...
<legend><h2 class="form-signin-heading">My Store</h2></legend>
      ...
      <?php echo $this->session->flashdata('response'); ?>  
         ...
         <label>Store Name:</label><input pattern="(?:[a-zA-Z]+[ ])+[a-zA-Z]+" type="text" required="required" name="store_name" class="input-block-level" value="<?php echo $get_store['store_name']; ?>">
         <label>Store Details:</label><input pattern="(?:[a-zA-Z]+[ ])+[a-zA-Z]+" type="text"  name="store_details" required="required" class="input-block-level" value="<?php echo $get_store['store_details'] ?>">
         <label>Zip Code:</label><input pattern="[0-9]+" type="text"  name="store_zip" required="required" class="input-block-level" value="<?php echo $get_store['store_zip'] ?>">
        ...
        <select name="store_category" required="required">
         ...
         <label>Store Logo:</label><input <?php if (!isset($get_store['store_logo'])) { ?> required="required" <?php } ?> type="file" id="store_logo" title="(Type: JPG,JPEG,PNG,GIF)" class="btn btn-file" name="store_logo">
